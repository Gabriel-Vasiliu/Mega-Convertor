...
import 'history/volume_history_page.dart';

const List<String> options = <String>['Liters', 'Gallons', 'Cubic meteres'];
...
class _VolumePageState extends State<VolumePage> {
  final String title = "Mega Converter - Volume";
  ...
  Future<void> setResult() async {
    if (double.tryParse(number) != null) {
      ...
      if (option1 == option2) {
        ...
        if (option2 == 'Gallons') {
          res = double.parse(number) * 0.2641;
        } else {
          //Cubic meteres
        ...
        if (option2 == 'Liters') {
          res = double.parse(number) * 3.7854;
        } else {
          //Cubic meteres
          res = double.parse(number) * 0.00378;
        }
      } else if (option1 == 'Cubic meteres') {
        if (option2 == 'Liters') {
          res = double.parse(number) * 1000;
        } else {
          //Gallons
          res = double.parse(number) * 264.17;
        }
      }
      setState(() {
      ...
      if (option1 != option2) {
        await File('lib/history/volumeHistory.txt').writeAsString(
      ...
      floatingActionButton: FloatingActionButton(
          ...
          onPressed: () {
            Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const VolumeHistoryPage(),
                ));
          }),
    );
  }
